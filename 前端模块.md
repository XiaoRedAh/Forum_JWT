...
**src/components/InteractButton.vue**
组件设计用到了props和emit，有一定的参考价值
```html
<script setup>
/*定义三个属性，外部传进来*/
defineProps({
  name: String, /*按钮未选中时的文本内容*/
  checkName: String, /*按钮选中时的文本内容*/
  color: String,
  check: Boolean /*该按钮是否被选中*/
})

const emit = defineEmits(['check']) /*配合到图标里的@click，点击图标时触发*/
</script>
...
**src/pages/forum/TopicDetail.vue**
文章详情中，添加点赞/收藏按钮
...
src/components/TopicCollectList.vue
组件设计用到了props和emit，有一定的参考价值
```html
<script setup>
import {get} from "@/net"
...
src/components/TopicCommentEditor.vue
组件设计用到了props和emit，有一定的参考价值
```html
<script setup>
import {
